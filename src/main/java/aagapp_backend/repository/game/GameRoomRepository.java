...
/*    @Query("SELECT SUM(g.maxPlayers) FROM GameRoom g WHERE g.game.id = :gameId")
    ...
    @Query("SELECT SUM(g.maxPlayers) FROM GameRoom g " +
            "WHERE g.game.id = :gameId " +
            "AND g.status = aagapp_backend.enums.GameRoomStatus.COMPLETED " +
            "AND g.gamepassword IS NOT NULL")
    Long sumMaxPlayersByGameIdWithCompletedStatusAndPassword(@Param("gameId") Long gameId);
